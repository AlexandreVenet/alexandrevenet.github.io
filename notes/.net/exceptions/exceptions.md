...
[Microsoft *Learn*](https://learn.microsoft.com/fr-fr/dotnet/api/system.exception _blank)
...
Dans le cas d'un **contrôle**, on peut utiliser `try...catch`. Si une instruction ne fonctionne pas dans `try`, le code de `catch` sera utilisé. `catch` interrompt le flux du programme. On peut laisser `catch` vide mais c'est déconseillé car on ne sait rien de l'erreur qui peut survenir. 
